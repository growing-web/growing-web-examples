...
const html = transformFragment(template, {
  get importmap() {
    ...
    const string = nodeEnv === 'development' ? JSON.stringify(importmap, null, 2) : JSON.stringify(importmap);
    return `
...
const string = await (nodeEnv === 'development' ? html : minify(html, { collapseWhitespace: true }));
